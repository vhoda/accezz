...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		foreach($result as $row)
		{
			...
			{
				$status = '<label class="badge text-bg-success">Presente</label>';
			}
			if($row["attendance_status"] == "Absent")
			{
				$status = '<label class="badge text-bg-danger">Ausente</label>';
			}
	...
	if($_POST["action"] == "index_fetch")
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '<button type="button" name="report_button" data-student_id="'.$row["student_id"].'" class="btn btn-dark btn-sm report_button">Resumir</button>
			';
			$data[] = $sub_array;
		}
