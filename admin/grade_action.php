...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '<button type="button" name="edit_grade" class="btn btn-primary btn-sm edit_grade" id="'.$row["grade_id"].'">Editar</button>';
			$sub_array[] = '<button type="button" name="delete_grade" class="btn btn-danger btn-sm delete_grade" id="'.$row["grade_id"].'">Eliminar</button>';
			$data[] = $sub_array;
		}
	...
	if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
	{
		...
		{
			$error_grade_name = 'El nombre de Curso es requerido';
			$error++;
		}
		else
		{
		...
		else
		{
			if($_POST["action"] == "Add")
			{
				...
				if($statement->execute($data))
				{
					...
					else
					{
						$output = array(
							'error'					=>	true,
							'error_grade_name'		=>	'Curso ya es existe'
						);
					}
				}
			}
			if($_POST["action"] == "Edit")
			{
				...
				{
					$output = array(
						'success'		=>	'Data Actualizado correctamente',
					);
				}
			}
		}
	...
	if($_POST["action"] == "delete")
	{
		...
		if($statement->execute())
		{
			echo 'Data eliminado Correctamente';
		}
	}
